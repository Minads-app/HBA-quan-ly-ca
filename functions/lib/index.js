...
exports.registerShift = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
admin.initializeApp();
// Shift registration function using HTTP Callable
exports.registerShift = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "User must be logged in to register for a shift.");
    }
    const { weekId, shiftId, role } = data;
    if (!weekId || !shiftId || !role) {
        throw new functions.https.HttpsError("invalid-argument", "Missing required fields: weekId, shiftId, role");
    }
    ...
    try {
        const result = await db.runTransaction(async (transaction) => {
            ...
            if (shiftData.staff && shiftData.staff[role] !== null) {
                throw new functions.https.HttpsError("already-exists", `The ${role} position is already taken.`);
            }
